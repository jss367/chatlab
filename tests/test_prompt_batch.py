...
class BatchCsvTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.addCleanup(shutil.rmtree, self.directory)
        self.addCleanup(os.umask, os.umask(0))
        self.table = BatchTable(self.directory)

    def add(self, trace, index):
        """Add a prompt the way a run does: its trace file, then the table.

        The table reads the traces back from those files when it has to
        write itself again, rather than holding a run's tokens in memory.
        """

        write_batch_trace(trace, self.directory, index)
        return self.table.add(trace, index)
    ...
    def test_each_prompt_adds_its_rows_to_the_one_table(self):
        self.add(sample_trace(seed=1), 1)
        path = self.add(sample_trace(seed=2), 3)
        rows = self.rows(path)
    ...
    def test_the_column_names_are_written_once(self):
        ...
        self.add(sample_trace(seed=1), 1)
        path = self.add(sample_trace(seed=2), 2)
    ...
    def test_a_prompt_is_not_written_again_by_the_next_one(self):
        ...
        self.add(sample_trace(seed=1), 1)
        first = Path(self.table.path).read_text(encoding="utf-8")

        self.add(sample_trace(seed=2), 2)
        after = Path(self.table.path).read_text(encoding="utf-8")

        self.assertTrue(after.startswith(first))

    def test_the_table_holds_no_tokens_between_prompts(self):
        # A run's tokens are the largest thing it produces, and holding them
        # until it ends would take that memory from the model. The table
        # keeps the prompt numbers and reads the traces back when it has to
        # write itself again.
        self.add(sample_trace(seed=1), 1)
        self.add(sample_trace(seed=2), 2)
        ...
        held = [
            value
            for value in vars(self.table).values()
            if isinstance(value, list) and any(isinstance(item, dict) for item in value)
        ]
        self.assertEqual(held, [])
        self.assertEqual(self.table.indexes, [1, 2])
    ...
    def test_a_wider_candidate_list_rewrites_the_table(self):
        ...
        self.add(sample_trace(seed=1, candidates=1), 1)
        path = self.add(sample_trace(seed=2, candidates=3), 2)
        rows = self.rows(path)
    ...
    def test_a_narrower_prompt_after_a_wide_one_keeps_the_columns(self):
        self.add(sample_trace(seed=1, candidates=3), 1)
        path = self.add(sample_trace(seed=2, candidates=1), 2)
        rows = self.rows(path)
    ...
    def test_the_table_is_owner_only_from_the_first_prompt(self):
        path = Path(self.add(sample_trace(), 1))

        self.assertEqual(stat.S_IMODE(path.stat().st_mode), 0o600)

        self.add(sample_trace(), 2)
