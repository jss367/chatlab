...
class MazeTests(unittest.TestCase):
    ...
    def two_response_run(self):
        """A live two-response episode whose every token records alternative 120."""
        move = call_text(MAZE.maze_id, "east")
        ids = list(move.encode()) + [0]
        episode = Episode(MAZE, CONFIG | {"interruption_text": "", "per_turn_tokens": 200, "token_budget": 1000})
        author = Manager([(move, ids)] * 2)
        author.generate = scored(author.generate)
        list(stream_episode(episode, author, single_step=True))
        list(stream_episode(episode, author, single_step=True))
        self.assertEqual(len(episode.turns), 2)
        self.assertNotIn(120, ids)
        return episode, author, move, ids

    def test_fork_refuses_an_alternative_on_a_response_an_earlier_fork_copied_in(self):
        """Forking an upload at a later response makes the episode live; its copies are not.

        The fork is stamped with the load in memory and carries the earlier
        responses of the uploaded run forward verbatim, alternatives included.
        Those responses were still produced by the uploaded session, so editing
        one of them afterwards must not read the new episode's stamp as evidence
        about the vocabulary that spelled them.
        """
        ep, author, move, ids = self.two_response_run()
        replay = from_payload(json.loads(json.dumps(ep.payload())))
        self.assertTrue(replay.replay_only)
        index = move.index("east")
        ...
        reloaded.load_id = "test-load#2"
        with reloaded.open_session() as session:
            forked = fork_token_edit(replay, 1, index, "west", session)
        self.assertFalse(forked.replay_only)
        self.assertEqual(forked.load_id, "test-load#2")
        self.assertTrue(forked.turns[0]["inherited"])

        # The second-generation edit: the episode satisfies the episode-level
        # question and the response it selects answers the other way.
        with reloaded.open_session() as session:
            with self.assertRaisesRegex(ValueError, "Replacement text"):
                fork_token_edit(forked, 0, index, "ignored", session, candidate_id=120)

        # Typing the branch still reaches it, encoded against the retained tokens.
        with reloaded.open_session() as session:
            typed = fork_token_edit(forked, 0, index, "x", session)
        self.assertEqual(typed.pending_edit["forced_ids"], ids[:index] + [120])

        # The response this fork generates itself carries no mark, so its own
        # alternatives were offered by the load in memory and stay available.
        suffix = move[index + len("east"):]
        reloaded.replies = iter([(suffix, list(suffix.encode()) + [0])])
        reloaded.generate = scored(reloaded.generate)
        list(stream_episode(forked, reloaded, single_step=True))
        self.assertEqual(len(forked.turns), 2)
        self.assertNotIn("inherited", forked.turns[1])
        start = forked.turns[1]["forced_prefix_tokens"]
        with reloaded.open_session() as session:
            again = fork_token_edit(forked, 1, start, "ignored", session, candidate_id=120)
        self.assertEqual(again.pending_edit["forced_ids"], ids[:index] + list(b"west") + [120])
        self.assertTrue(again.turns[0]["inherited"])

    def test_fork_of_a_live_episode_keeps_the_alternatives_of_the_responses_it_copies(self):
        """The copies of a same-session fork were generated by the load in memory."""
        ep, author, move, ids = self.two_response_run()
        index = move.index("east")
        with author.open_session() as session:
            forked = fork_token_edit(ep, 1, index, "west", session)
        self.assertEqual(forked.load_id, ep.load_id)
        self.assertNotIn("inherited", forked.turns[0])
        with author.open_session() as session:
            again = fork_token_edit(forked, 0, index, "ignored", session, candidate_id=120)
        self.assertEqual(again.pending_edit["forced_ids"], ids[:index] + [120])

    def test_the_inherited_mark_survives_export_and_upload(self):
        """A fork of a fork, exported and uploaded, still refuses the copied alternatives."""
        ep, author, move, ids = self.two_response_run()
        replay = from_payload(json.loads(json.dumps(ep.payload())))
        index = move.index("east")
        reloaded = Manager([])
        ...
        saved = json.loads(json.dumps(forked.payload()))
        self.assertTrue(saved["turns"][0]["inherited"])
        self.assertNotIn("inherited", saved["turns"][1])
        uploaded = from_payload(saved)
        self.assertTrue(uploaded.turns[0]["inherited"])
        with reloaded.open_session() as session:
            with self.assertRaisesRegex(ValueError, "Replacement text"):
                fork_token_edit(uploaded, 0, index, "ignored", session, candidate_id=120)

        # Forking the upload again carries the mark another generation on, so
        # the copied response stays refused in the live episode that results.
        with reloaded.open_session() as session:
            refork = fork_token_edit(uploaded, 1, forked.turns[1]["forced_prefix_tokens"], "x", session)
        self.assertTrue(refork.turns[0]["inherited"])
        with reloaded.open_session() as session:
            with self.assertRaisesRegex(ValueError, "Replacement text"):
                fork_token_edit(refork, 0, index, "ignored", session, candidate_id=120)
