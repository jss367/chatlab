...
The cache is read from memory, so moving **Cache layer** or changing the readout costs no pass. A reply, a scoring pass, another inspection or a steered inspection releases it, and the view then asks for **Inspect layers** again. A sliding-window layer holds only its latest tokens and shows only those rows. A hybrid model's state-space layers hold no keys and say so. A layer holding more than 1,024 positions shows the latest 1,024. On MLX the memory figure is what each layer has allocated, which runs a little past the tokens held.
