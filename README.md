...
- LoRA adapters from the Hub, downloaded with the base model they were trained on and merged into it, so a fine-tuned persona answers in chat with every measurement intact
...
A LoRA adapter is a small set of weights trained on top of a text model, and it is how most personas and behavior fine-tunes are published: a repository with an `adapter_config.json`, which names the base model, and an `adapter_model.safetensors` a few tens of megabytes long. Enter the adapter's ID on the Models page like any other model. **Download and load** fetches the adapter, reads which base it names, and fetches that too, using the same access token, since popular bases are often gated. **Load cached** then reads the base and merges the adapter into it, adding each low-rank update into the weight it was trained against. What ends up in memory is a plain Transformers model under the adapter's ID. The token view, logit lens, attention view, steering, extraction, and patching all work on it unchanged, and **Comparing two runs** can put the adapter beside its base.

Merging needs the full-precision weights, so an adapter always loads at full precision, whatever **Weight precision** says. It takes the memory of the whole base model, and the fit verdicts and the load's memory check measure the base. The ready card and the log say when a 4-bit or 8-bit choice was passed over. An adapter that ships its own tokenizer, because training added tokens or a chat template, is read with that tokenizer, and the base's embeddings are grown to match it.

Adapters trained with Unsloth usually name Unsloth's bitsandbytes 4-bit copy of a model (`unsloth/Qwen2.5-7B-Instruct-bnb-4bit`) as their base. bitsandbytes does not run on Apple silicon, and a packed matrix cannot take a merge anyway, so ChatLab loads the full-precision repository that copy was made from (`unsloth/Qwen2.5-7B-Instruct`), and the adapter fits it. **My Models** marks an adapter *LoRA* and names its base. An adapter whose base is missing is listed as incomplete. It is marked unsupported, with the reason, when it is a type other than LoRA (prompt or prefix tuning add no weights to merge into), when it was trained for something other than text generation, or when its base is a local path from the training machine, an MLX conversion, an image model, or another adapter. Adapters load through PyTorch only. mlx-lm's own adapter format is not read.
