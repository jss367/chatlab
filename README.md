...
The sampling controls are **not** here. Temperature, top-p, top-k, **Skip top choice below**, the response length and the seed are what gets moved between one retry and the next, so they sit under the message box on the Chat page, in a **Sampling** section that wears its own values: the summary reads without opening it. **Reset to defaults** at the foot of that section puts the five back where the app ships them - temperature 0.8, top-p 0.95, top-k 50, no top-choice skip, 1,024 new tokens, or the context limit if it is lower - which is the way back from an experiment without having to remember where you started. It counts as moving the sliders by hand, so it is written into the conversation and the settings file like any other move. The seed is left alone: a seed being held to reproduce a reply is not a setting to be put back.
