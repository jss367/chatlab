...
The sampling controls are **not** here. Temperature, top-p, top-k, **Skip top choice below**, the response length and the seed are what gets moved between one retry and the next, so they sit under the message box on the Chat page, in a **Sampling** section that wears its own values: the summary reads without opening it. The ↺ beside each slider puts that one control back to its default - temperature 0.8, top-p 0.95, top-k 50, no top-choice skip, 1,024 new tokens - which is the way back from an experiment without having to remember where you started. It counts as moving the slider by hand, so it is written into the conversation and the settings file like any other move.
