...
An uploaded run itself stays read-only: Run, Step and **Interrupt next response** all refuse it, and its file is never rewritten, so a stale snapshot cannot overwrite a newer archive of the same run. Editing a token is the way into a live episode. Editing needs the same model ID loaded, not the same load: reloading those weights, or loading them in a later session, is enough as long as that load tokenizes the run the same way, because what the fork replays is token IDs. Supplied prefix tokens remain read-only. Loading a *different* model requires starting a new episode. A fork continues under the weights in memory at the time, so when those are a later load it first verifies that the retained tokens still decode to the text they recorded, and refuses the edit if they do not — that is how a re-download of the same repository ID at a revision with a different tokenizer is caught rather than silently rewriting the retained prefix. The run JSON keeps both stamps.
