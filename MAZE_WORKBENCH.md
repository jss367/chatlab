...
7. Select a response under **Path and replay** to inspect its board and tokens, or walk the episode with **◀ Previous** and **Next ▶**. **Play from here** advances from the selected response to the last one at **Playback seconds per response**, and **Stop playback** ends it; **Previous** and **Next** still work afterwards. Playback only redraws this view, so it works on a finished episode and on a saved replay, which is what **Run episode** refuses to do. Pause a live episode before stepping or playing it back. Export JSON or upload a previous export for read-only replay.
