...
An uploaded run itself stays read-only: Run, Step and **Interrupt next response** all refuse it, and its file is never rewritten, so a stale snapshot cannot overwrite a newer archive of the same run. Editing a token is the way into a live episode. Editing needs the same model ID loaded, not the same load: reloading those weights, or loading them in a later session, is enough, because what the fork replays is token IDs and the tokenizer comes from the model ID. Supplied prefix tokens remain read-only. Loading a *different* model requires starting a new episode. A fork continues under the weights in memory at the time, which may be a different snapshot of that repository ID than the one that produced the original; the run JSON keeps both stamps.
