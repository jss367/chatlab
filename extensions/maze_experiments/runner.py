...
def fork_token_edit(episode, turn_index, token_index, replacement, manager, *, candidate_id=None):
    ...
    with episode.lock:
        ...
        # The fork replays stored token IDs, so the tokenizer has to match; a
        # later load of the same repository ID supplies it. load_id is not
        # required, which is what lets an uploaded run be forked at all.
